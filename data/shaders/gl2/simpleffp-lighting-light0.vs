...
void computeLighting()
{
    ...
    {
        Acm = bdfx_frontMaterial.diffuse;
    ...
    vec4 lightResult = internalDiffuse( VPli,
        Dcm, bdfx_lightSource[ idx ].diffuse );

    vec4 specResult = vec4( 0., 0., 0., 0. );
    specResult += internalSpecular( VPli.xyz, bdfx_lightSource[ idx ].specular );

    vec3 totalAmbient = Acm.rgb * bdfx_lightModel.ambient.rgb;
    bdfx_processedColor =
        vec4( totalAmbient, 0. ) +
