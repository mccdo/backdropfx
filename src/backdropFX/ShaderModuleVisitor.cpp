...
ShaderModuleVisitor::convertStateSet( osg::StateSet* ss )
{
	...
	for(int texUnit = 0; texUnit < BDFX_MAX_TEXTURE_UNITS; texUnit++)
	{
		...
		const char *samplerUnitNames[] = {"texture2dSampler0", "texture2dSampler1", "texture2dSampler2", "texture2dSampler3"}; // out to BDFX_MAX_TEXTURE_UNITS
	...
	const char *texgenUnitNames[] = {"bdfx_texGen0", "bdfx_texGen1", "bdfx_texGen2", "bdfx_texGen3"}; // out to BDFX_MAX_TEXTURE_UNITS
...
ShaderModuleVisitor::setDefaults( osg::Node& node )
{
    ...
    // Light model
    {
        ...
        lightModel = new osg::Uniform( "bdfx_lightModel.ambient", osg::Vec4f( .1f, .1f, .1f, 1.f ) );
        ss->addUniform( lightModel );
        lightModel = new osg::Uniform( "bdfx_lightModel.localViewer", 0 );
        ss->addUniform( lightModel );
        lightModel = new osg::Uniform( "bdfx_lightModel.separateSpecular", 0 );
        ss->addUniform( lightModel );
        lightModel = new osg::Uniform( "bdfx_lightModel.twoSided", 0 );
        ss->addUniform( lightModel );
    }
